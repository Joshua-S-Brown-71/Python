...
def training_step(model, X_batch, y_batch, reg):
    ...
    # Compute the loss
    data_loss, d_scores = softmax_loss(scores, y_batch)
    reg_loss, d_reg = l2_regularization(model.params['W1'], reg), reg * model.params['W1']
    loss = data_loss + reg_loss
    # Backward pass
    grads = model.backward(d_scores, cache)
    grads['W1'] += d_reg
...
def plot_stats(stats, filename):
    if 'loss' not in stats or 't' not in stats:
        print("Error: 'loss' or 't' not found in stats dictionary")
        return

    loss_data = stats['loss']
    t_data = stats['t']

    if len(loss_data) != len(t_data):
        print("Error: 'loss' and 't' data have different lengths")
        return

    if not all(isinstance(val, (int, float)) for val in loss_data):
        print("Error: 'loss' data contains non-numeric values")
        return

    if not all(isinstance(val, (int, float)) for val in t_data):
        print("Error: 't' data contains non-numeric values")
        return

    plt.subplot(1, 2, 1)
    plt.plot(t_data, loss_data, 'o', alpha=0.5, ms=4)
    plt.title('Loss')
    plt.xlabel('Epoch')
    loss_xlim = plt.xlim()
